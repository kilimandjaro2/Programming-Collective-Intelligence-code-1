...
if DEBUG:
	print('Set socket timeout to %s seconds' \
		% DLCS_REQUEST_TIMEOUT, file=sys.stderr)
...
class _Waiter:
	...
	def __call__(self):
		...
		if timeago <= self.wait:
			if DEBUG > 0:
				print('Waiting %s seconds.' % self.wait, file=sys.stderr)
...
def str2quote(s):
	return urllib.parse.quote_plus(''.join([chr(ord(i)).encode('utf-8') for i in
							 s]))
...
def dlcs_api_request(
	...
	if DEBUG:
		print('dlcs_api_request: %s' % url, file=sys.stderr)
...
.... {'result':(True, "done")}
	...
	if DEBUG > 3:
		print('dlcs_parse_xml: parsing from ', data, file=sys.stderr)
...
#	 for e in rss.entries: print e;print
	...
	for e in rss.entries:
		if 'links' in e and e['links'] != [] and 'href' in e['links'
				][0]:
			url = e['links'][0]['href']
		elif 'link' in e:
			url = e['link']
